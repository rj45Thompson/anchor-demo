...
- [ ] R10b Enemy bolt + heavy boss bolt become retro sprites -> DONE WHEN: the enemy bolt (`SphereGeometry(0.20,8,6)` now :987) and heavy boss bolt (`SphereGeometry(1.04,18,14)` now :989) are additive camera-facing sprites (reuse the `_glowPool` / `glowSprite` from R11), POOLED not `new`-ed; screenshotted before/after and ms/frame reported both ways. ⚠ bolts live in `bolts[]` with `.m`; despawn sites at :1070, :2482 and the bolt loop :1041.
