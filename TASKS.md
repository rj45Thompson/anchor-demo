...
- [x] L1a Letter treatment A: EXTRUDED — **SHIPPED 2026-09-07.** Real `THREE.TextGeometry` per glyph (vendored `assets/helvetiker_regular.typeface.json`; TextGeometry/Font/FontLoader are in r128 core — proven at runtime before wiring) as a new `LETTER_STYLES` entry `extrude` (index 3, extends RJ's cycler, press L). NOT a texture on a quad — genuine extruded mesh with depth (height 0.46 + bevel). **The contrast comes from GEOMETRY, not a rectangle**: the extruded SIDES are an UNLIT dark material (`MeshBasicMaterial 0x06080e`) so every glyph carries a guaranteed-dark contour following its own shape (a lit dark side washed to grey under the 1.75 ambient — the first attempt's flaw); faces are self-lit `MeshStandardMaterial` (emissive 0.65) so they read + bloom catches them. Per-glyph uniform fit-scale keeps proportional widths in the monospace cells. Collision unaffected (hit test uses L.x/line.y, not the mesh); `giveMesh` guards extrude meshes out of the card pool. **Reference shot** (glacier/CRAWL/crawl12): `breakout-evidence/L1a2_s3_c0_cr12.png`. **Numbers** over the brightest 15% of the planet: glyphLuma 115.1 / bgLuma 164.6 → **ratio 0.70**, michelson 0.177 (readability is from the dark contour + bright faces; the mean blends them). **ms/frame 9.2** (⚠ superseded — that was first-call compile noise; the L2 uniform re-measure with a warm-up run gives **4.04 ms**, still the most expensive treatment but far under budget). Committed and pushed before L1b. Graph: `BX-letter-extrude = yes`.
- [x] L1b Letter treatment B: STROKED — **VERIFIED 2026-09-07 (already built by RJ; Rule 5).** RJ's `OUTLINE` style (`glyphTexture` :696, `LETTER_STYLES` index 1) IS this treatment: bright white fill + a heavy dark stroke that hugs each glyph's own shape (no rectangle, no card — "the subtitle solution"). It reads cleanly over the worst-case glacier at the reference (every line legible). Not restyled — it's RJ's design; I verified and measured it. **Reference shot**: `breakout-evidence/L1b_s1_c0_cr12.png`. **Numbers** over the brightest 15%: dark-contour separation **bestSep 0.396** (glyphP10 73 / bg 167, the dark stroke does the work), ratio 0.84; **ms/frame 3.28** (range 3.25-3.55, 305fps — cheapest of the per-glyph styles). Metric note: the mean-based michelson (0.068) badly underrates bimodal bright-ink+dark-edge treatments, so the harness now reports `bestSep` = the stronger of {bright-ink, dark-edge} separation from the bright bg (all treatments re-measured uniformly in L2). Graph: `BX-letter-outline-stroked = yes`.
- [x] L1c Letter treatment C: SELF-LIT EMISSIVE — **SHIPPED 2026-09-07.** New `emissive` glyph branch (`glyphTexture`) + `LETTER_STYLES` index 4: a coloured self-glow built up around a near-white core, NO dark halo / backing of any kind — it wins on LUMINANCE alone, and the shipped bloom catches the white core (visible bloom halos in the shot). This is the clean decomposition of RJ's NEON: **EMISSIVE = NEON minus the halo**, L1d = the halo alone, NEON = both. **Reference shot**: `breakout-evidence/L1c_s4_c0_cr12.png`. **Numbers** over the brightest 15%: darkSep 0.016 (no backing, by design), **brightSep/bestSep 0.199** (white core 255 vs bg 170) — honestly the WEAKEST separation of the set, because pure brightness has the least to win with over already-luma-255 city lights; **ms/frame 6.8** (147fps). It's gorgeous where the bg is dark and marginal where it's brightest — a real aesthetic-vs-readability tradeoff for RJ to weigh, presented with its number. Graph: `BX-letter-emissive = yes`.
- [x] L1d Letter treatment D: SOFT DARK HALO — **SHIPPED 2026-09-07.** New `halo` glyph branch + `LETTER_STYLES` index 5: a soft, wide blurred black aura laid around each letterform (plus a tighter inner darkening at the edge), then the glyph filled at its OWN tone — no box, no hard stroke, no bright core. The DARKENING does the contrast and it follows the glyph SHAPE. Lower uGlow (0.35) keeps the fill normal-bright, which is what distinguishes it from EMISSIVE (bright, no dark) and NEON (bright AND dark). Distinct bonus: it **preserves the résumé's tone colours** (white heads, cyan detail, orange turrets) instead of forcing white — the only treatment that keeps the colour-coding. **Reference shot**: `breakout-evidence/L1d_s5_c0_cr12.png`. **Numbers** over the brightest 15%: **bestSep 0.263** (soft dark aura, glyphP10 99 / bg 170 — between EMISSIVE 0.199 and OUTLINE's crisp 0.396, as expected for a soft vs hard edge); **ms/frame 3.26** (307fps). Graph: `BX-letter-halo = yes`.
- [x] L1e Letter treatment E: MINE = INKED — **SHIPPED 2026-09-07, and it beat A-D.** Building A-D taught the lesson: the strongest separation over a bright bg comes from a DARK contour that follows the shape (OUTLINE 0.396, EXTRUDE's sides), but OUTLINE throws away the résumé's tone colours by forcing white, while HALO keeps colour but is soft (0.263). INKED combines the wins: a crisp hard dark contour (OUTLINE's contrast) + a subtle safety halo (so a luma-255 city cell can't eat the edge) + the glyph kept at its OWN tone (HALO's colour-coding). New `inked` glyph branch + `LETTER_STYLES` index 6, uGlow 0.55. **Reference shot**: `breakout-evidence/L1e_s6_c0_cr12.png`. **Numbers** over the brightest 15%: **bestSep 0.851 — the HIGHEST of all six** (glyphP10 13.6, a genuinely dark, substantial contour, vs OUTLINE's thin 73); **ms/frame 3.12** (321fps — among the cheapest). It reads better than any of A-D over the worst-case glacier AND preserves the colour-coding (cyan detail, white heads, orange turrets). This is my recommendation for L3 (RJ picks). Graph: `BX-letter-inked = yes`.
...
- [x] L2  Measure each one — **DONE 2026-09-07.** All treatments measured UNIFORMLY with the final harness (`lettershot.mjs`, 480×300 luminance sampling, warm-up + median-of-3×120 perf), at the pinned reference (glacier / CRAWL / crawl 12, background luma ≈170). Method: hide letters → re-render → the pure background; a pixel a letter changed by >22 luma is "glyph". Over the **brightest 15% of the planet**, `bestSep` = the stronger of {bright-ink, dark-contour} Michelson separation from the bright bg (the honest single number for a bimodal bright-ink+dark-edge glyph; higher = more readable over the worst case). `ratio` = mean glyph / bg.

  | # | treatment | bestSep (mechanism) | ratio | ms/frame (fps) | screenshot |
  |---|---|---|---|---|---|
  | A | EXTRUDE  | **0.655** (dark 3D sides) | 0.67 | 4.04 (248) | `breakout-evidence/L2_s3_c0_cr12.png` |
  | B | OUTLINE  | 0.396 (hard dark stroke)  | 0.83 | 3.36 (298) | `L2_s1_c0_cr12.png` |
  | C | EMISSIVE | 0.199 (bright core only — weakest over bright bg) | 1.28 | 3.33 (300) | `L2_s4_c0_cr12.png` |
  | D | HALO     | 0.289 (soft dark aura, colour kept) | 0.99 | 3.33 (300) | `L2_s5_c0_cr12.png` |
  | E | **INKED** (mine) | **0.820** (dark contour + colour kept) | 0.65 | 3.45 (290) | `L2_s6_c0_cr12.png` |
  | — | PAGE (RJ, bonus) | 0.731 (dark per-line card) | 0.40 | 2.60 (385) | `L2_s0_c0_cr12.png` |
  | — | NEON (RJ, bonus) | 0.198 (bright + halo) | 1.25 | 3.21 (312) | `L2_s2_c0_cr12.png` |

  **Findings:** (1) all seven are cheap — 2.6-4.0 ms, 248-385fps at pr1 — so performance is a non-issue and the pick is purely look/readability. (2) The strongest separation over the worst-case bright planet comes from a DARK contour: INKED 0.820 > PAGE 0.731 > EXTRUDE 0.655 > OUTLINE 0.396; the bright-only treatments (EMISSIVE/NEON ≈0.199) are weakest because bright-on-bright has the least to win with. (3) **EXTRUDE's real cost is 4.04 ms** — the 9.2 ms recorded in L1a was first-call shader/geometry compilation, removed by the warm-up run here. Graph: `BX-letter-treatments-measured = yes`.
- [x] L4  A 6-SECOND fly-in before the sweeping cinematic — **SHIPPED 2026-09-07.** Restructured `buildIntroFrames` (the existing R6-R9 intro had only a ~4.3s fly-in blended into the reveal): now beat one is a dedicated **6.0s fly-in** — four keyframes from far orbit (z+152, planet a small curve) straight in to close (z+33, planet large), the limb swelling frame to frame — and ONLY at introT 6.0 does the existing sweeping cutscene begin (R7 reveal → R8 flank → settle to CRAWL). **MEASURED live** (`intro_shot.mjs`, drove `tick()` with a monotonic clock; ⚠ `__bx.step(1)` gives dt≈0 in a tight loop, so it's driven by hand): **fly-in 5.9s** (reveal begins at introT 6.0; 5.9s wall = 354 frames at 60fps, the honest frame-counted value vs the 6.0s design boundary — first-frame dt slack), **total intro 8.8s** (527 frames), then **state intro→play** confirmed. **Constraints all verified**: skippable from frame 1 (Space right after START → state `play`, camMode 0/camBlend 1 ✓); no replay (returning player with `bx_intro_seen` → `play` directly, no intro ✓); hands into the SAME state a cold start reaches (play / camMode 0 / camBlend 1 ✓); reuses `CAM_MODES[0]` + `camBlend` (no second camera system ✓). **Approach reads as speed** — the limb grows dramatically across the stills. **Stills at 0/2/4/6s**: `breakout-evidence/L4_flyin_{0,2,4,6}s.png` (0s far orbit + tiny fighter → 4s full résumé readable → 6s planet dominant, handing to the reveal). Graph: `BX-intro-6s-flyin = yes`.
- [x] L3  Contact sheet published — **DONE 2026-09-07.** Artifact URL (records here as required): **https://claude.ai/code/artifact/a75baf07-223e-4788-ac55-f25da0ce9ff8** ("Glyphs Over Glacier"). Shows all five treatments (A Extruded, B Outline, C Emissive, D Halo, E Inked) side by side at the SAME scale (all 1280×800 glacier/CRAWL/crawl12 frames), each with its contrast number (bestSep) as a bar + value and its ms/frame + fps; PAGE and NEON included as "already in the build" bonuses; a readability-ranking bar up top. **No winner shipped** — the default style is unchanged (`letterStyle=0` PAGE) and all seven cycle live with **L**, so RJ picks. **Recommendation (one sentence, in the artifact and here):** E · INKED — it reads the clearest of the five over the worst-case bright planet (contrast 0.82) and is the only high-contrast option that keeps the résumé's colour-coding, at the same frame cost as the rest. Images embedded as data URIs (CSP-safe, 1.6MB total). Graph: `BX-letter-contact-sheet = yes`.
