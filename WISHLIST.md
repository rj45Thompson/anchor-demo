...
| ⭐⭐ B45: "how many moons does Jupiter have" answered about a children's book instead | ✅ | found running the FULL 100-question cardinality zoo, not by hand - `findEntities` resolved the subject as "Many Moons" (a real James Thurber book/Janelle Monáe song, 23 facts) for Jupiter/Saturn/Neptune/Earth, and "Many Rings" for Saturn's rings. B30 excluded quantifiers ("many") from being linked as an ENTIRE span, but never taught the leading/trailing TRIM loop the same rule, so "many" survived as the first word of the core span and happened to complete a real, longer, coincidentally-matching entity - same shape as B2's "Country Is". Fixed: the trim loop's own gate now checks the same `QUANT` set, one line. Verified: all five previously-affected questions now resolve the real astronomical concept and still get B44's ask-back framing correctly. `findEntities` is shared by every lane - unchanged golden traces are the real evidence nothing else shifted |
